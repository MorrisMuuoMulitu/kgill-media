...
const FeaturedReels: React.FC<FeaturedReelsProps> = ({ reels }) => {
  ...
  // Get 3 random reels excluding the ones shown in the featured carousel
  const getRandomReels = () => {
    // Get all reels that are not in the featured carousel
    const nonCarouselReels = reels.filter(reel => !reel.featured);
    
    // If we have non-featured reels, use them; otherwise use all reels
      ...
      <section className="py-20 bg-gradient-to-br from-charcoal via-slate-900 to-black relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-6xl font-black font-montserrat mb-6 epic-text">
              KGILL TV Highlights
            </h2>
            
            <p className="text-xl md:text-2xl text-gray-400 font-inter max-w-3xl mx-auto leading-relaxed">
              Curated selection of our most engaging video content
            </p>
          </div>
          
          {/* Featured Reel Showcase */}
          <div className="relative">
            ...
            <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-white/20 hover:border-gold-gradient-start/50 transition-all duration-500 mb-12">
              <div className="aspect-video bg-gradient-to-br from-marigold/20 to-terracotta/20 rounded-3xl flex items-center justify-center">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  ...
                  <h3 className="text-2xl md:text-3xl font-bold font-montserrat text-white mb-2 truncate">
                    {currentReel?.title}
                  </h3>
                  <p className="text-gray-300 font-inter line-clamp-2">
              ...
              <div className="absolute top-6 left-6 flex gap-3">
                <div className="px-3 py-1.5 bg-slate-900/90 backdrop-blur-xl rounded-full border border-white/10 shadow-lg">
                  <div className="flex items-center gap-1.5 text-gray-300 font-inter text-xs font-semibold">
                    <Eye className="w-3.5 h-3.5 text-gold-gradient-start" />
            ...
            <div className="flex justify-between items-center">
              <button
                onClick={prevReel}
                className="flex items-center gap-3 px-6 py-3 bg-white/10 hover:bg-white/20 border-2 border-white/20 hover:border-gold-gradient-start/50 rounded-xl text-white font-bold text-sm transition-all duration-300 group"
              >
          ...
          <div className="mt-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {randomReels.map((reel) => (
                <div 
                  key={reel.id}
                  className="group relative overflow-hidden rounded-2xl shadow-lg hover:shadow-2xl border border-white/10 hover:border-cyan/50 transition-all duration-300 cursor-pointer"
                ...
                >
                  ...
                  <div className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="text-lg font-bold font-montserrat text-white truncate">
                        {reel.title}
                      </h4>
                      <span className="flex-shrink-0 px-2 py-1 bg-gold-gradient/20 text-gold-gradient-start rounded-lg text-xs font-bold ml-2">
                        {reel.category}
                      </span>
                    </div>
                    
                    <p className="text-gray-400 font-inter text-sm line-clamp-2 mb-3">
                      {reel.description}
                    </p>
