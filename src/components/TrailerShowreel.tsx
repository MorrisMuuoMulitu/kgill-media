...
const TrailerShowreel: React.FC<TrailerShowreelProps> = ({ videos }) => {
  ...
  const trailers = videos && videos.length > 0 ? videos : defaultTrailers;
  ...
  }, [nextTrailer, trailers.length]);

  const current = trailers[currentTrailer % trailers.length];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
            <div className={`text-left ${trailerChange ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'} transition-all duration-500`}>
              ...
              <div className="flex flex-wrap gap-4 mb-8">
                <button onClick={() => openVideo(current)} className="btn-primary flex items-center gap-3 premium-hover-gold">
                  <Play className="w-5 h-5 transition-transform group-hover:scale-110" />
                  <span>Watch Video</span>
                </button>
                <a href="/kgill-tv" className="btn-secondary flex items-center gap-3">
            ...
            <div className="glass-premium rounded-3xl p-6 border border-white/10 backdrop-blur-xl">
              ...
              <div className="space-y-4">
                {trailers.map((trailer, index) => (
                  <div
                    ...
                    className={`flex gap-4 p-4 rounded-2xl cursor-pointer transition-all duration-300 premium-hover ${index === currentTrailer
                    ...
                    onClick={() => { setCurrentTrailer(index); openVideo(trailer); }}
                  >
                    <div className="relative flex-shrink-0">
                      <img src={trailer.thumbnail} alt={trailer.title} className="w-24 h-16 object-cover rounded-xl" />
              ...
              <div className="flex justify-between mt-6">
                <button onClick={prevTrailer} className="px-6 py-3 rounded-xl bg-white/10 hover:bg-white/20 transition-colors font-bold">Previous</button>
                <button onClick={nextTrailer} className="px-6 py-3 rounded-xl bg-white/10 hover:bg-white/20 transition-colors font-bold">Next</button>
              </div>
            </div>
          </div>
        </div>

        <button onClick={() => openVideo(current)} className="absolute bottom-8 right-8 z-10 px-6 py-3 rounded-full bg-gold-gradient text-white backdrop-blur-lg border border-gold-gradient-start/30 hover:scale-105 transition-all premium-hover epic-glow font-bold flex items-center gap-2">
